...
项目不仅追求能跑通，更强调工程表达力。最终交付覆盖了接口设计、前后端联调、缓存机制、阿里云 FC3 Serverless 部署配置，以及面向场景的可读性和可维护性。
演示地址https://wang54.github.io/resume326/
...
项目深度应用 Vibecoding 理念，将大模型真正纳入工程生产流程，而不是仅停留在辅助写几段代码的层面。实现过程中，大模型被用于：
...
项目没有停留在缓存已经做完的表面，而是进一步讨论了其在 Serverless 运行环境中的局限：

- 当前缓存属于进程内内存缓存
- 冷启动后缓存会丢失
- 多实例并发时缓存无法天然共享

未来演进方案非常明确：将缓存抽离到 Redis 或其他外部 KV 存储，以获得跨实例共享、高可用和可观测能力。
### 4. 真实可替换的大模型调用链路
